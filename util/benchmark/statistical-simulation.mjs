...
function configuration(seed = 1, actualBlocks = 80) {
  return {
...
export function simulatedArtifact(
  ...
  const blocks = [];
  const schedule = createComparisonSchedule(blockCount, 'simulation-seed');
  for (let index = 0; index < blockCount; index++) {
    const baselineFirst = schedule[index].processOrder[0] === 'baseline';
...
function adaptiveArtifact(scenario, seed, resamples) {
  const full = simulatedArtifact(scenario, seed, resamples);
  for (
    let blockCount = full.configuration.minimumBlocks;
    blockCount <= 80;
    blockCount += 2
  ) {
    const blocks = full.blocks.slice(0, blockCount);
    const interim = interimTotalAnalysis(blocks, {
      ...full.configuration,
      actualBlocks: blockCount,
    });
    if (
      interim?.confidenceIntervalWidth <= full.configuration.precisionTarget
    ) {
      return {
        ...full,
        configuration: { ...full.configuration, actualBlocks: blockCount },
        schedule: full.schedule.slice(0, blockCount),
        blocks,
      };
    }
  }
...
if (process.argv[1]?.endsWith('/statistical-simulation.mjs')) {
  ...
  const bands = {
    ...
    // Optional stopping is calibrated separately: normal-data coverage stays
    // near nominal, while the intentionally skewed stress case may under-cover.
    adaptiveIntervalCoverage: { target: 0.95, low: 0.85, high: 0.99 },
