...
test('effective benchmark configuration changes its provenance hash', () => {
  const defaults = repositoryHashes(process.cwd(), {
    mode: 'stable',
    warmup: 20,
    iters: 100,
    preset: 'default',
  ...
  const changed = repositoryHashes(process.cwd(), {
    mode: 'stable',
    warmup: 20,
    iters: 100,
    preset: 'default',
  ...
  assert.notEqual(defaults.benchmarkConfigHash, changed.benchmarkConfigHash);
});
...
test('provenance hash domains are explicit and non-overlapping', (t) => {
  ...
  // Workflow files are not part of the injected harness; their changes must
  // not invalidate comparisons copied between worktrees.
