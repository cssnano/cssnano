...
export function loadSnapshot(arg) {
  ...
  if (snapshot.schemaVersion === 3) {
    ...
    for (const [name, hash] of hashes) {
      if (snapshot.outputHashes?.[name] !== hash) {
        throw new Error(
          `v3 aggregate output hash disagrees with runs: "${arg}"`
        );
      }
    }
    exactNumber(
  ...
  if (snapshot.schemaVersion === 2) {
    ...
    for (const run of snapshot.runs) {
      ...
      if (run.summary.total.medianMs <= 0) {
        throw new RangeError(
          `benchmark run summary total.medianMs must be positive numbers; received ${run.summary.total.medianMs} in "${arg}"`
        );
      }
      validateSampleArray(run.totalSamples, `run ${run.index} total`);
      if (run.perFileSamples !== undefined) {
        if (!run.perFileSamples || typeof run.perFileSamples !== 'object') {
          throw new TypeError(
            `invalid per-file benchmark samples in snapshot: "${arg}"`
          );
        }
        for (const [name, samples] of Object.entries(run.perFileSamples)) {
          validateSampleArray(samples, `run ${run.index} ${name}`);
        }
      }
    }
    validateFrameworks(snapshot, arg);
    return { ...snapshot, path, legacy: true };
  }
