 ...
 *
 * Removal can fail when Git metadata is read-only; the caller is then told
 * exactly how to recover instead of being told cleanup succeeded.
 *
 * @param {string} targetPath - Worktree path to remove.
 * @param {object} [options]
 * @param {string} [options.repoRoot] - Repository root directory.
 * @param {Function} [options.runner] - Process runner for git commands.
 * @returns {{ removed: boolean, path: string, recoveryCommand?: string, error?: string }}
 */
export function cleanupWorktree(targetPath, options = {}) {
  ...
  const recoveryCommand =
    `chmod -R u+w ${resolvedPath} 2>/dev/null; rm -rf ${resolvedPath}; ` +
  ...
  // The directory was removed; drop stale worktree administrative metadata.
  // A prune failure must still be surfaced: without it, `git worktree list`
  // keeps reporting a worktree whose directory no longer exists.
  let pruneError = null;
  try {
    runner('git', ['worktree', 'prune'], { cwd: repoRoot });
  } catch (error) {
    pruneError = String(error.message ?? error);
  }
  ...
  return pruneError
    ? {
        ...
        pruneError,
        recoveryCommand,
        error: `worktree directory removed, but git worktree prune failed: ${pruneError}`,
      }
    : { removed: true, path: resolvedPath };
}
...
function main() {
  ...
  if (args.cleanup) {
    ...
    if (result.removed) {
      ...
      console.error(
        `failed to clean up worktree at ${result.path}: ${result.error ?? 'unknown error'}\n` +
