...
//
// Runs the default preset over every CSS file in ../frameworks for a fixed
// number of warmup + measured iterations and prints a per-framework + total
// summary. Writes a JSON snapshot to ../bench-results/<label>.json so different
// branches/optimizations can be compared.
//
// Usage:
//   node util/bench.mjs                    # label = "baseline"
...
//   node util/bench.mjs --only=bootstrap   # substring filter on framework name
...
function parseArgs(argv) {
  const out = { label: 'baseline', iters: 15, warmup: 3, only: null };
...
async function main() {
  ...
  for (const f of files) {
    const source = readFileSync(join(FRAMEWORKS_DIR, f), 'utf8');
