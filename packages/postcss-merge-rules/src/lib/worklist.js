...
export default function runWorklist(root, api) {
  ...
  const reseedCandidates = (initial = false) => {
    ...
    // Seeding refreshes every rule and bumps its version, so all previously
    // queued edge keys are permanently stale; drop them instead of letting
    // the set grow without bound across reseeds.
    queuedEdges.clear();
