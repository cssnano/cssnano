...
test('should drop queued edges on a global reseed so they can be queued again', () => {
  ...
  try {
    runWorklist(root, {
      ...
      mergeMatchingDeclarations: () => {
        declarationMerges++;
        // The movedAcrossParents flag makes the worklist request a global
        // reseed, which clears the queued-edge set and must allow the same
        // rule pairs to be queued again.
        return declarationMerges === 1
          ? {
              previous: null,
              replacements: [second],
