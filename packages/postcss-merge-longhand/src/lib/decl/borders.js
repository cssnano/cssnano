...
function merge (rule) {
    ...
    rule.walkDecls(/^border($|-(top|right|bottom|left)$)/i, decl => {
        ...
        wsc.forEach((d, i) => {
            ...
            mergeRules(rule, [decl.prop, ...props], (rules) => {
                ...
                if (longhands[0].value.toLowerCase() === longhands[1].value.toLowerCase() &&
                    longhands[1].value.toLowerCase() === longhands[2].value.toLowerCase() &&
    ...
    while (decls.length) {
        ...
        const lesser = decls.filter(node =>
            ...
            (!!~node.prop.toLowerCase().indexOf(lastNode.prop) || node.prop.toLowerCase().endsWith(lastPart)));
