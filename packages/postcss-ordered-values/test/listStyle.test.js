...
test(
  'orders list-style with author-defined custom counter styles, position, and image',
  processCSS(
    'a{list-style:inside url(icon.svg) my-bullet;list-style:my-bullet inside url(icon.svg)}',
    'a{list-style:my-bullet inside url(icon.svg);list-style:my-bullet inside url(icon.svg)}'
  )
);
...
describe('Order', () => {
  ...
  test(
    'should classify and reorder quoted URLs as list-style images',
    processCSS(
      'a{list-style:inside url("icon.svg") none}',
      'a{list-style:none inside url("icon.svg")}'
    )
