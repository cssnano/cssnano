...
const malformedCases = new Set([
  'comment-abort',
  'substitution-abort',
  'css-loader-abort',
  'quoted-url',
  'escaped-string',
  'malformed-processable',
]);

test('abort and malformed cases preserve declaration values byte-for-byte', () => {
  for (const sample of edgeCases.filter((s) => malformedCases.has(s.name))) {
    const output = process(sample.css);
    assert.equal(output, sample.css, sample.name);
  }
});
