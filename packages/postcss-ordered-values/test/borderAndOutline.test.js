...
test(
  'should fail closed and pass through border with auto',
  passthroughCSS('h1{border:solid auto 1px}')
);

test('preserves existing raws.value reference when declaration is already in canonical order', async () => {
  const rawsValue = { raw: '1px solid red', value: '1px solid red' };
  const decl = postcss.decl({ prop: 'border', value: '1px solid red' });
  decl.raws.value = rawsValue;
  const root = postcss.root({ nodes: [decl] });
  await postcss([plugin()]).process(root, { from: undefined });

  assert.strictEqual(decl.raws.value, rawsValue);
  assert.strictEqual(decl.value, '1px solid red');
});

describe('Property lookup', () => {
  test(
    'should order case-insensitive vendor-prefixed properties',
    processCSS(
      'h1{-WEBKIT-BORDER:solid red 1px;-webkit-border:solid red 1px}',
      'h1{-WEBKIT-BORDER:1px solid red;-webkit-border:1px solid red}'
    )
  );
  ...
  test(
    ...
    passthroughCSS(
      'h1{--border:solid red 1px;--BORDER:solid red 1px;--webkit-border:solid red 1px}'
    )
  );

  test(
    'should pass through border declarations with structural tokens',
    passthroughCSS(
      'a{border: (foo) 1px solid;border: [foo] 1px solid;border: @foo 1px solid;border: 1px solid (foo)}'
    )
  );

  test(
    'should pass through border with case-insensitive CSS-wide keywords',
    passthroughCSS('h1{border: solid INHERIT 1px;border: INITIAL 1px solid}')
  );
});
