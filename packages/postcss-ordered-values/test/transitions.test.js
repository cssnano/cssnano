...
test(
  'should pass through transition declarations with structural tokens',
  passthroughCSS(
    'a{transition: 1s #foo ease;transition: 1s (foo) ease;transition: 1s [foo] ease;transition: 1s @foo ease}'
  )
);

test(
  'should pass through multi-item transition containing none',
  passthroughCSS(
    'a{transition: 1s none, opacity 2s;transition: opacity 2s, 1s none;transition: none, transform 1s;transition: none, none}'
  )
);

test(
  'orders single-item transition containing none',
  processCSS('a{transition: 1s none ease}', 'a{transition: none 1s ease}')
);

test(
  'orders transition with negative math duration per CSS Values 4 range clamping',
  processCSS(
    'a{transition: calc(-1s) opacity ease;transition: ease calc(-1s) opacity}',
    'a{transition: opacity calc(-1s) ease;transition: opacity calc(-1s) ease}'
  )
);
