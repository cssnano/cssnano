...
describe('Tokenization', () => {
  ...
  test('tokenization classifies url terms', () => {
    const parsed = tokenizeValue(
      'url("foo.png") url(bar.png) other("foo") ident'
    );
    assert.strictEqual(isUrl(parsed.terms[0]), true);
    assert.strictEqual(isUrl(parsed.terms[1]), true);
    assert.strictEqual(isUrl(parsed.terms[2]), false);
    assert.strictEqual(isUrl(parsed.terms[3]), false);
  });

  test('tokenization classifies delimiter terms and character matching', () => {
    const parsed = tokenizeValue('10px / 20px');
    assert.strictEqual(isDelim(parsed.terms[0]), false);
    assert.strictEqual(isDelim(parsed.terms[1]), true);
    assert.strictEqual(isDelim(parsed.terms[1], '/'), true);
    assert.strictEqual(isDelim(parsed.terms[1], ','), false);
    assert.strictEqual(isDelim(parsed.terms[2]), false);
  });

  test('tokenization aborts on malformed strings and urls', () => {
    assert.strictEqual(tokenizeValue('"unterminated\n10px').abort, true);
    assert.strictEqual(tokenizeValue('url(foo"bar)').abort, true);
  });
});
