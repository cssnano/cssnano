...
import { serializeComponentValues } from '../lib/parse.js';
import joinGridValue from '../lib/joinGridValue.js';

const valuesOf = (nodes) =>
  nodes
...
const normalizeGridAutoFlow = (gridAutoFlow) => {
  ...
  if (
    ...
    return serializeComponentValues(gridAutoFlow);
...
const normalizeGridColumnRowGap = (gridGap) => {
  ...
  return values.includes('normal')
    ...
    : serializeComponentValues(gridGap);
};
...
const normalizeGridColumnRow = (grid) => {
  ...
  for (const node of grid) {
    if (isTokenNode(node) && isTokenDelim(node.value) && node.value[1] === '/')
      lines.push([]);
  ...
  return lines.length > 1
    ? joinGridValue(lines.map(normalizeLine))
    : serializeComponentValues(grid);
};
