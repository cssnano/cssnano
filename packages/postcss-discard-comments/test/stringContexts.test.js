...
describe('String contexts and URLs', () => {
  ...
  test(
    'should preserve comment-like text in unquoted URLs',
    passthroughCSS('h1{background:url(http://x/*y*/)}')
  );

  test(
    'should preserve comment-like text in unquoted data URLs',
    passthroughCSS('h1{background:url(data:image/png;base64,iVBOR/*x*/)}')
  );

  test(
    'should preserve escaped comment delimiters behind escapes in strings',
    passthroughCSS('a{content:"x\\/*y*/"}')
  );

  test(
    'should preserve escaped slashes in unquoted URLs',
    passthroughCSS('a{background:url(x\\/*y*/z)}')
  );
});
