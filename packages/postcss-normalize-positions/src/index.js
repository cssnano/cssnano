...
function transform(value) {
  ...
  args.forEach((arg) => {
    ...
    arg.forEach((part, index) => {
      const isPosition =
        ~directions.indexOf(part.value.toLowerCase()) || unit(part.value);
  ...
  relevant.forEach((range, index) => {
    ...
    const secondValue =
      position[2] && position[2].value ? position[2].value.toLowerCase() : null;
...
export default plugin('postcss-normalize-positions', () => {
  return (css) => {
    ...
    css.walkDecls(
      /^(background(-position)?|(-webkit-)?perspective-origin)$/i,
      (decl) => {
