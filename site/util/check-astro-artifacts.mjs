...
async function findUnprefixedInternalUrls(directory) {
  const violations = [];
...
const unprefixedInternalUrls =
  await findUnprefixedInternalUrls(outputDirectory);
if (unprefixedInternalUrls.length > 0)
  throw new Error(
    `Found unprefixed internal URLs in: ${unprefixedInternalUrls.join(', ')}`
  );

const playground = await readFile(
  path.join(outputDirectory, 'playground/index.html'),
  'utf8'
);
const assets = await readdir(assetDirectory);
const worker = assets.find(
  (file) => file.startsWith('playground-worker-') && file.endsWith('.js')
);
...
if (imports.length !== 3)
  throw new Error(
    `Expected three lazy preset imports, found ${imports.length}`
  );
for (const preset of [
  ...
  if (!workerSource.includes(preset))
    throw new Error(`Missing ${preset} from the Astro worker`);
}
if (!playground.includes(worker))
  throw new Error('Playground page does not reference the Astro worker');
for (const importedAsset of imports) {
  if (!assets.includes(path.basename(importedAsset)))
