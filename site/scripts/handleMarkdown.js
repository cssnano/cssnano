...
function getMetadata () {
    return toml.parse(
        fs.readFileSync(join(__dirname, '../../metadata.toml'), 'utf8')
    );
}
    ...
    return Promise.resolve()
        ...
        .then((changlog) => {
            return new Promise((resolve) => remark()
                ...
                // Bug in this.Parser.prototype, need solve
...
function updateOptimisationGuide () {
    return Promise.resolve()
        .then(() => fs.readFile(join(__dirname, '../content/guides/optimisations.md')))
        .then((optimisations) => {
...
function updateOptimisationsIndex () {
    return Promise.resolve()
        ...
        .then((packages) => {
            ...
            return remark().stringify(optimisationsTable(presets, plugins));
        })
        .then(
...
function updateCommunity () {
    return Promise.resolve()
        ...
        .then((transformedContributors) => {
            return Promise.resolve()
                .then(() => fs.readFile(join(__dirname, '../content/community.md')))
                        ...
                        .use(() => tree => {
                            heading(tree, /Contributors/, (start, nodes, end) => {
                                return [
                                    ...
                                    ...remark().parse(transformedContributors).children,
                                    end,
                                ];
                            })
                        })
